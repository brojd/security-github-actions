...
### Advisory check
A check whose result depends on external state (registry, advisory database, time of day). The same commit can pass on Monday and fail on Friday because a new CVE was published overnight. Doesn't gate merge: surfaces via PR comment to nudge the author, but `continue-on-error: true` keeps it from gating merge. Example: `registry-audit` (runs `npm/pnpm/yarn audit`).

### Ecosystem
A language/package-manager world the workflow knows how to inspect. PR2 ships only `js` (Node.js — npm / pnpm / yarn); the type system already models it as a discriminated union so a follow-up PR can add `go` (Go modules) without reshaping consumers. Each ecosystem has its own walker, its own checks, and its own root type. The engine pairs a check with a root only when the ecosystems match. Adding a new ecosystem means a new walker, a new root variant, and a new `checks/<eco>/` directory — not a plugin system.

### Manifest
The file the walker keys on to discover roots. PR2 keys only on `package.json`; a follow-up PR adds `go.mod`. The presence of *any* manifest activates the supply-chain workflow.
...
### Lockfile conflict
A root containing more than one lockfile (e.g. both `package-lock.json` and `pnpm-lock.yaml`). Almost always indicates a half-finished migration. Treated as a **hard fail** — the workflow refuses to guess which manager rules to apply, and the developer must resolve it explicitly. Enforced by the `lockfile-conflict` check.
